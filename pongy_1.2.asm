;PONGY! V1.2 (C) METESEV 2023
;WRITTEN BETWEEN 20jAN23-14FEB23

;MEMORY MAP OF THE GAME
;$0801 - $0810   PROGRAM START
;$0820 - $0ED0   MAIN PROGRAM
;$OED6 - $0F74   VARIABLES AND TABLES
...
; -----------------------------> THIS DELAY IS FOR NORMAL SPEED
                                ; SPEED FOR THE FIRST 50 SECONDS
DELAY_NORMAL                
         LDA #$00
         STA $21D0
         STA $21D1
...
;-------------------------------> THIS DELAY IS FOR FASTER SPEED 
                                ; SPEED 2 FOR THE LAST 00 SECONDS
...
; ------------------------------> THIS DELAY IS FOR FASTEST SPEED 
                                 ; SPEED FOR THE LAST 10 SECONDS
DELAY_FASTEST                               
        LDA $D850              
        CMP #$02        
        BEQ FASTEST

        LDX #$00                ; CHECK IF THE FIRST CHARACTER OF THE LINE IS RED
RED     LDA #$02                ; IF IT IS ALREADY RED
        STA $D850,X             ; NO NEED TO DRAW RED LINES AND GO FASTEST ROUTINE
        STA $DB98,X
        INX
        CPX #$28
        BNE RED
...
PLACESPRITES
        ...
        LDX BALLDIRECTION       ; IF THE BALL IS UP RIGHT
URIGHT  CPX #$03                ; SET THE MSB FOR THE BALL
        BNE DRIGHT
        LDA SPRITEPOS+4
        CMP #$00
        BNE ENDPLACE
...
DRIGHT  CPX #$04                ; IF THE BALL IS DOWN RIGHT
        BNE DLEFT               ; SET THE MSB FOR THE BALL
        LDA SPRITEPOS+4
        CMP #$00       
        BNE ENDPLACE
        LDA #%00000100
...
DLEFT   CPX #$02                ; IF THE BALL IS DOWN LEFT       
        BNE ULEFT               ; CLEAR THE MSB
        LDA SPRITEPOS+4
        CMP #$FF       
        BNE ENDPLACE
        LDA #%11111001
...
ULEFT   LDA SPRITEPOS+4         ; IF THE BALL IS UP LEFT
        CMP #$FF                ; CLEAR THE MSB
        BNE ENDPLACE
        LDA #%11111001
        AND $D010
        STA $D010        
...
WAIT_BUTTON
        ...
        LSR A                   ; CHECK PADDLE 2 BUTTON
        BCC EXITB               ; IF PRESSED, EXIT WAITING BUTTON

        JSR COLORWASH           ; OTHERWISE, COLORWASH THE TEST ON THE SCREEN      

RLOOP   LDA $D012               ; SHORT DELAY ->WAIT RASTER
        BNE RLOOP

        JMP WAIT_BUTTON         ; GO BACK AND WAIT BUTTON AGAIN
...
CHECK_CONTROLS_P1
        ...
        STX SPRITEPOS+1         ; STORE IT ON THE POSITION
        LDX SPRITEPOS+1
...
CHECK_CONTROLS_P2
        ...
        STY SPRITEPOS+3         ; STORE IT ON THE POSITION
        LDY SPRITEPOS+3
